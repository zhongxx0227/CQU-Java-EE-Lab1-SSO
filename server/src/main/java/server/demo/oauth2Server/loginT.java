...
public class loginT {
    ...
    public String loginHandle(HttpServletRequest request, HttpServletResponse response, Model model)
    {
        ...
        System.out.println("到达loginT！");
        String username = request.getParameter("username");
        System.out.println("loginT username "+username);
        String token = security.decrypt(request.getParameter("token"));
        System.out.println("loginT token " + token);
        String newToken = "-1";
        // 获取验证token的消息体
        ...
        if(ans.code==102)
        {
            ...
            System.out.println("loginT now: "+now);
            System.out.println("loginT tokenTime: "+tokenTime);
            // 如果token快要过期了，重新签发一个
